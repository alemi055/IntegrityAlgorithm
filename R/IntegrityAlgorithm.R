...
# MAIN FUNCTION 7
Clonality_Analysis <- function(){
  # (bool) -> None
  #
  # Input:
  #   - RefSeq: logical. If TRUE, the reference sequence is included in the FASTA file.
  #
  ...
  suppressWarnings(
    for (i in fasta){
      ...
      # Look at seqs with less than 5 differences - if 0 = clone, but if between 1 and 5 = potential clones
      ...
      for (j in 1:length(seqs_names)){
        name <- seqs_names[j]
        pos <- as.numeric(which(pairwise_matrix[j,] <= 5))
        ...
        if (length(pos) > 1){ # If length(pos) = 1, it is only the pairwise comparison with itself. It needs to be at least 2 for a duplicate.
          ...
          for (k in pos){
            ...
            }else if (pairwise_matrix[j,k] >= 1 & pairwise_matrix[j,k] <= 5){ # potential seq
