...
IntegrateInfo <- function(filename){
  ...
  if (!(all(sapply(list(manual_excel$Name, ProseqIT_excel$Name, QCTool_excel$SeqName, GC_excel$Name), FUN = identical, manual_excel$Name)))){
    ...
    message1 <- paste0("\nThe number of sequences is not the same in all files. Please verify each file individually.\n-> Analyzed_QCTool.csv (in 'tmp' folder) = ", nrow(QCTool_excel),
...
check_integer <- function(analyzes, threshold){
  ...
  for (i in analyzes){
    if (i != 1 & i != 2 & i != 3 & i != 4 & i != 5){
      stop("\nThe value(s) provided for the argument \'analyzes\' is not an integer between 1 and 5.")
